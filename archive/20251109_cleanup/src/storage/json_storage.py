...
class JSONStorage:
    ...
    def save_categories(self, categories: Dict[str, Any]):
        ...
        with open(categories_file, 'w', encoding='utf-8') as f:
            json.dump(categories, f, ensure_ascii=False, indent=2, cls=DateTimeEncoder)
    ...
    def save_snapshot(self, category_tree: Dict[str, Any], total_articles: int):
        ...
        with open(snapshot_file, 'w', encoding='utf-8') as f:
            json.dump(snapshot_data, f, ensure_ascii=False, indent=2, cls=DateTimeEncoder)

        logger.info(f"分类快照已保存: {snapshot_file}")

    def save_failed_file(self, file_path: str, error: str, error_type: str = None):
        """保存失败文件信息"""
        ...
        if failed_log.exists():
            try:
                ...
                logger.warning(f"读取失败日志文件出错: {e}，将创建新文件")
                failed_data = []
        ...
        if error_type:
            failed_entry["error_type"] = error_type
        ...
        try:
            ...
            logger.debug(f"失败记录已保存: {file_path}")
        except Exception as e:
